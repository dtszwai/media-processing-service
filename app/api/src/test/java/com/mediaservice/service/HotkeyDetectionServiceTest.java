...
import com.mediaservice.dto.analytics.MediaViewCount;
  ...
  class WarmUpFromAnalytics {
    ...
    void shouldLoadHotKeysFromAnalyticsOnRefresh() {
      ...
      when(analyticsService.getTopMedia(Period.TODAY, 100)).thenReturn(List.of(
          MediaViewCount.builder().mediaId("media1").viewCount(1000L).build(),
          MediaViewCount.builder().mediaId("media2").viewCount(500L).build()));
      when(analyticsService.getTopMedia(Period.ALL_TIME, 100)).thenReturn(List.of(
          MediaViewCount.builder().mediaId("media1").viewCount(5000L).build(),
          MediaViewCount.builder().mediaId("media3").viewCount(3000L).build()));
      when(stringRedisTemplate.opsForSet()).thenReturn(setOperations);
